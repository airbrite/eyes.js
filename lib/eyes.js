...
function stringify(obj, options) {
  ...
  result = (function(obj) {
    switch (typeOf(obj)) {
      ...
      case "date":
        return stylize(obj.toString(), 'date');
      case "array":
...
function stringifyArray(ary, options, level) {
  var out = [];
  var pretty = options.pretty && (ary.length > 0 || ary.some(function(o) {
  ...
  if (out.length === 0) {
    ...
    return '[' + ws + out.join(',' + (pretty ? ws : ' ')) + (pretty ? ws.slice(0, -2) : ws) +
      ']';
  }
}
...
function stringifyObject(obj, options, level) {
  var out = [];
  var pretty = options.pretty && (Object.keys(obj).length > 0 ||
    Object.keys(obj).some(function(k) {
      return typeof(obj[k]) === 'object';
    }));
  var ws = pretty ? '\n' + new(Array)(level * 2 + 1).join(' ') : ' ';

  var keys = options.showHidden ? Object.getOwnPropertyNames(obj) : Object.keys(obj);
